  ...
  return (
    <main className="min-h-screen bg-[#FDFBF7] text-[#1A1A1A] selection:bg-gold-500/30 overflow-x-hidden font-sans">
      ...
      <section className="relative min-h-screen flex flex-col items-center justify-center pt-16 px-6 z-10 text-center">
        ...
        >
          {/* Expansive, soft background glow */}
        ...
        {/* Elegant, Slimmer Buttons - WIDTH CONSTRAINED */}
        <motion.div 
          initial={{ y: 20, opacity: 0 }} 
          animate={{ y: 0, opacity: 1 }} 
          transition={{ delay: 0.4 }}
          /* Changed to items-center to prevent stretching, and added sm:flex-row for tablet/desktop */
          className="mt-14 flex flex-col sm:flex-row items-center justify-center gap-4 w-full"
        >
          <motion.button whileTap={{ scale: 0.95 }} className="bg-[#181818] text-white h-12 md:h-14 w-[260px] sm:w-auto px-10 rounded-full text-[10px] md:text-xs font-bold uppercase tracking-[0.2em] hover:bg-gold-700 transition-all shadow-xl">
            Latest Analysis
          </motion.button>
          <motion.button whileTap={{ scale: 0.95 }} className="bg-transparent border border-black/15 text-[#111] h-12 md:h-14 w-[260px] sm:w-auto px-10 rounded-full text-[10px] md:text-xs font-bold uppercase tracking-[0.2em] hover:bg-black/5 transition-all">
            Media Kit
          </motion.button>
        </motion.div>
      ...
      {/* EXPERTISE: THE "EDITORIAL FEATURE" LOCKUP (MOBILE-FIRST) */}
          ...
          <div className="flex flex-col">
            {expertise.map((item, i) => (
              <motion.div 
                ...
                whileTap={{ scale: 0.98 }} // PERFECT MOBILE TACTILE
              ...
              >
                {/* TOP ROW: Icon + Tag (Left) & Chapter Number (Right) */}
                ...
                {/* MOB FIRST: Interactive Arrow visible on mobile, hidden on desktop until hover */}
      ...
      {/* MEDIA WALL: CINEMATIC EDITORIAL GRID */}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-10 md:gap-8">
          {appearances.map((item) => (
            <motion.div 
              ...
              whileTap={{ scale: 0.97 }} // PERFECT MOBILE TACTILE
            >
              {/* Cinematic Thumbnail Area */}
              <div className="aspect-video bg-[#111] rounded-[2rem] overflow-hidden relative mb-6 shadow-sm md:group-hover:shadow-[0_20px_40px_rgba(0,0,0,0.15)] transition-shadow duration-700">
                
                {/* The Actual YouTube Thumbnail with Ken Burns & Bloom. MOB FIRST: Full color on mobile */}
                <img 
                ...
                {/* MOB FIRST: Elegant Play Button (Solid gold on mobile, frosted glass on desktop) */}
                <div className="absolute inset-0 flex items-center justify-center">
                  <div className="w-16 h-16 rounded-full flex items-center justify-center shadow-2xl transition-all duration-500 bg-gold-500 border-gold-500 md:border-white/30 md:bg-black/10 md:backdrop-blur-md md:group-hover:scale-110 md:group-hover:bg-gold-500 md:group-hover:border-gold-500">
                    <Play className="ml-1 transition-colors duration-500 text-black fill-black md:text-white md:fill-transparent md:group-hover:text-black md:group-hover:fill-black" size={24} />
                  </div>
                </div>

                {/* Floating 'Watch' Badge */}
                <div className="absolute top-4 left-4 bg-white/10 backdrop-blur-md px-3 py-1 rounded-full border border-white/20 opacity-0 md:group-hover:opacity-100 -translate-y-2 md:group-hover:translate-y-0 transition-all duration-500">
              ...
              <div className="flex flex-col border-l-2 border-gold-500 md:border-transparent md:group-hover:border-gold-500 pl-4 md:pl-0 md:group-hover:pl-4 transition-all duration-300">
                ...
                <h4 className="text-2xl font-serif leading-snug text-[#111] md:group-hover:text-gold-700 transition-colors">
                  {item.title}
                </h4>
              </div>
            </motion.div>
      ...
      {/* INSIGHTS: MUSEUM GALLERY STYLE */}
      <section id="insights" className="bg-[#111] text-white py-32 mt-12 rounded-t-[4rem] z-10 relative shadow-[0_-20px_50px_rgba(0,0,0,0.1)]">
        <div className="max-w-7xl mx-auto px-6 grid md:grid-cols-2 gap-12">
          
          {/* Gallery Header */}
          <div className="md:col-span-2 mb-10 border-b border-white/10 pb-8 flex flex-col md:flex-row justify-between items-start md:items-end">
            <div>
          ...
          {[
            ...
            >
              ...
              {/* The "Print" Frame */}
              <div className="relative w-full aspect-[4/5] bg-[#1a1a1a] rounded-[2rem] overflow-hidden mb-6 border border-white/5 shadow-lg md:group-hover:shadow-2xl transition-all duration-700 p-4 md:p-6">
                ...
                <div className="relative w-full h-full rounded-xl overflow-hidden bg-black ring-1 ring-white/10 shadow-inner">
                  ...
                  {/* MOB FIRST: Full color on mobile */}
                  <img 
                    src={img.src} 
                  ...
                  {/* MOB FIRST: Center Expand Button visible on mobile */}
              ...
              <div className="flex flex-col pl-4 border-l-2 border-gold-500 md:border-transparent md:group-hover:border-gold-500 transition-all duration-500">
                ...
                <h4 className="text-3xl font-serif text-white md:group-hover:text-gold-400 transition-colors duration-300">
                  {img.title}
                </h4>
              </div>
            </motion.div>
        ...
        <div className="max-w-7xl mx-auto px-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-16">
            {articles.map((article) => (
              <motion.div 
                key={article.id} 
                className="group border-t border-black/10 pt-8 cursor-pointer rounded-xl -mx-4 px-4 pb-4 md:hover:bg-black/[0.02] transition-colors"
                whileTap={{ scale: 0.98 }} // PERFECT MOBILE TACTILE
              >
                <p className="text-[10px] font-black text-gold-600 uppercase tracking-widest mb-3">
                  {article.publisher} • {article.published_date}
                </p>
                <h3 className="text-2xl md:text-4xl font-serif leading-tight md:group-hover:text-gold-700 transition-colors mb-4">
                  {article.title}
                </h3>
                <p className="text-slate-600 leading-relaxed font-light italic">
                  "{article.excerpt}"
                </p>
                {/* MOB FIRST: Arrow always visible on mobile */}
      ...
      <footer className="bg-white py-24 px-6 text-center border-t border-black/5">
        <motion.button whileTap={{ scale: 0.95 }} className="inline-flex items-center gap-3 bg-[#1A1A1A] text-white hover:bg-gold-600 px-10 py-5 rounded-full font-bold shadow-xl transition-all">
        ...
        {selectedImg && (
          ...
          >
            ...
            <motion.img 
              ...
              exit={{ scale: 0.9, y: 20 }}
              src={selectedImg} 
              className="max-h-[85vh] w-auto rounded-xl shadow-2xl ring-1 ring-white/10" 
